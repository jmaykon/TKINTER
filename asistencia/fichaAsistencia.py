...
class CAsistencia(Frame):  # Herencia Frame <-- Ventana
    ...
    st = 0
    #hoy = ''
    def __init__(self, master=None):  # Contructor de Ventana geometry('1345x675+0+0')
        super().__init__(master, width=1345, height=675)  # Herencia constructor de Frame = master
    ...
    def create_widgets(self):
        ...
        frame1.place(x=0, y=0, width=1345, height=675)

        self.re = PhotoImage(file='img/IMG11.png')  # FONDO DENTRO DEL FRAME
        Label(frame1, image=self.re).place(x=-10, y=-10)  # LA POCISION DE LA IAMGEN DENTRO DEL FRAME
        ...
        self.btnBuscar.place(x=380, y=95, width=50, height=30)
        # Hora Total
        ...
        self.txt_ap_pat.place(x=380, y=162)
        self.cajaTxt_ap_pat.place(x=510, y=162)

        self.txt_ap_mat = Label(frame1, text="APELLIDO MATERNO :")
        self.cajaTxt_ap_mat = Entry(frame1, font="15", width=15)
        self.txt_ap_mat.place(x=700, y=162)
        self.cajaTxt_ap_mat.place(x=850, y=162)
        ...
        self.txt_area.place(x=380, y=222)
        self.cajaTxt_area.place(x=510, y=220)

        self.txt_turno = Label(frame1, text="TURNO :")
        self.cajaTxt_turno = Entry(frame1, font="15", width=15)
        self.txt_turno.place(x=700, y=222)
        self.cajaTxt_turno.place(x=850, y=220)
        ...
        self.btnGuardarS.place(x=360, y=320, width=80, height=30)
        ...
        self.titulo2 = Label(frame1, text="RECESO", font=("Arial Black", 15), fg="black", bg="orange")
        self.titulo2.place(x=380, y=420)

        ##CAJAS HORA
        self.cajaTxt_horaS2 = Entry(frame1, font="15", width=15)
        self.cajaTxt_horaS2.place(x=180, y=480)
        self.btnGuardarS2 = Button(frame1, text="Hora Salida", command=self.fGuardarS2, bg='#063374',
                                   fg="white")  # fNuevo
        self.btnGuardarS2.place(x=80, y=480, width=80, height=30)

        ##BUTTON LLEGADA
        self.btnGuardarLL = Button(frame1, text="Hora Llegada", command=self.fGuardarLL, bg="red", fg="white")  # fNuevo
        self.btnGuardarLL.place(x=360, y=480, width=80, height=30)

        ##CAJA LLEGADA
        self.cajaTxt_horaLL = Entry(frame1, font="15", width=15)
        self.cajaTxt_horaLL.place(x=450, y=480)

        ##############################################################
        # BTN TIEMPO ACUMULADO  2
        self.txt_D2 = Label(frame1, text="Tiempo Acumulado:")
        self.txt_D2.place(x=630, y=480)

        # CAJA TIEMPO ACUMULADO  2
        self.cajaTxt_horaD2 = Entry(frame1, font="15", width=10)
        self.cajaTxt_horaD2.place(x=760, y=480)

        self.btnLimpiar = Button(frame1, text="Limpiar", command=self.flimpiar, bg="brown", fg="white")  # fNuevo
        self.btnLimpiar.place(x=450, y=95, width=50, height=30)
    ...
    def fGuardarS2(self):
        if self.swBB == 1:
            ...
            if self.cajaTxt_horaS2.get():
                self.cajaTxt_horaS2.delete(0, END)
                self.cajaTxt_horaS2.insert(0, a)
                print("verdad")
                self.a2.modificar_asistencia(self.cajaTxt_ci.get(), self.cajaTxt_horaS2.get(), self.cajaTxt_horaS2.get(),
                                             self.cajaTxt_horaD2.get(), self.cajaTxt_fechaA.get())
                ...
                self.cajaTxt_horaS2.insert(0, a)
                print("falso")
                self.a2.registra_asistencia(self.cajaTxt_ci.get(), self.cajaTxt_horaS2.get(), self.cajaTxt_horaLL.get(),
                                            self.cajaTxt_horaD2.get(), self.cajaTxt_fechaA.get())
    ...
    def fGuardarLL(self):
        if self.swBB == 1:
            ...
            a = ahora.strftime("%H:%M") + ""
            self.horatS = a
            self.habilitarHoraLL('normal')
            self.limpiarHoraLL()
            self.cajaTxt_horaLL.insert(0, a)
            hh2 = self.hs * 60
            hm2 = hh2 + self.ms
            ...
            datosH1 = self.a2.buscar_asistencia_nomFe(self.cajaTxt_ci.get(), self.hoy)
            ...
            sumaH = str(nh), ':', str(nm)
            self.cajaTxt_horaD2.configure(state='normal')
            self.cajaTxt_horaD2.delete(0, END)
            self.cajaTxt_horaD2.insert(0, sumaH)
            self.cajaTxt_horaD2.configure(state='disabled')

            print(self.hoy)
            print(type(self.hoy))  #

            datosA = self.a2.buscar_asistencia_nomFe(self.cajaTxt_ci.get(), self.hoy)  # hoy cambiar
            print(datosA)
            self.cajaTxt_horaS2.delete(0, END)  # reseteamos la casilla
            self.cajaTxt_horaS2.insert(0, datosA[1])  # entra
            # self.cajaTxt_fechaA.insert(0, datosA[4])  # dfecha
            # self.btnGuardarS.configure(state="normal")#---

            datos = self.a2.modificar_asistencia(self.cajaTxt_ci.get(), self.cajaTxt_horaS2.get(),
                                                 self.cajaTxt_horaLL.get(),
                                                 self.cajaTxt_horaD2.get(), self.cajaTxt_fechaA.get())
    ...
    def fGuardar_HoraT2(self):
        datosD = self.t2.buscar_HoraDia(self.cajaTxt_ci.get())
        total_h = 0
        total_m = 0
        for fila in datosD:
            dato_hd = fila
    ...
    def fbuscarCi(self):
        if self.cajaTxt_ci.get() != "":
            ...
            try:
                ...
                self.habilitarHoraD('normal')
                self.limpiarHoraE()

                self.cajaTxt_horaE.insert(0, datosCA[1])
                self.cajaTxt_horaS.insert(0, datosCA[2])
                self.cajaTxt_horaD.insert(0, datosCA[3])
                self.habilitarHoraE('disabled')
                self.habilitarHoraS('disabled')
                self.habilitarHoraD('disabled')

                #********************************************************************
                self.habilitarHoraS2('normal')
                self.habilitarHoraLL('normal')
                self.habilitarHoraD2('normal')

                self.cajaTxt_horaS2.insert(0, datosRE[1])
                self.cajaTxt_horaLL.insert(0, datosRE[2])
                self.cajaTxt_horaD2.insert(0, datosRE[3])

                self.habilitarHoraS2('disabled')
                self.habilitarHoraLL('disabled')
                self.habilitarHoraD2('disabled')
    ...
    def habilitarCaja(self, estado):
        ...
        self.cajaTxt_horaS2.configure(state=estado)
        self.cajaTxt_horaLL.configure(state=estado)
        self.cajaTxt_horaD2.configure(state=estado)

    def habilitarHoraE(self,estado):
        self.cajaTxt_horaE.configure(state=estado)
    def habilitarHoraS(self, estado):
        self.cajaTxt_horaS.configure(state=estado)
    def habilitarHoraD(self, estado):
        self.cajaTxt_horaD.configure(state=estado)
    #***********************************************************************************
    def habilitarHoraD2(self, estado):
        self.cajaTxt_horaD2.configure(state=estado)
    def habilitarHoraS2(self,estado):
        self.cajaTxt_horaS2.configure(state=estado)
    def habilitarHoraLL(self, estado):
        self.cajaTxt_horaLL.configure(state=estado)
    #////////////////////////////////////////////////////////////////////////////////
    def limpiarHoraE(self):
        self.cajaTxt_horaE.delete(0, END)

    def limpiarHoraLL(self):
        self.cajaTxt_horaLL.delete(0, END)

    def limpiarHoraS2(self):
        self.cajaTxt_horaS2.delete(0, END)
    def limpiarHoraD2(self):
        self.cajaTxt_horaD2.delete(0, END)
