...
class Registro(Frame):  # Herencia Frame <-- Ventana
    ...
    def __init__(self, master=None):  # Contructor de Ventana
        super().__init__(master, width=1335, height=675)  # Herencia constructor de Frame = master
    ...
    def fGuardar(self):
        if self.id == -1:
            self.p1.registra_pasante(self.cajaTxt_ci.get(), self.cajaTxt_nom.get(), self.cajaTxt_ap_pat.get(), self.cajaTxt_ap_mat.get(), self.cajaTxt_carrera.get(), self.cajaTxt_area.get(), self.cajaTxt_cel.get(), self.cajaTxt_fechaIni.get(), self.cajaTxt_fechaFin.get(), self.cajaTxt_turno.get(), self.cajaTxt_gestion.get())
            messagebox.showinfo("Insertar", "Elemento insertado correctamente")
        else:
            self.p1.modificar_pasante(self.id, self.cajaTxt_ci.get(), self.cajaTxt_nom.get(), self.cajaTxt_ap_pat.get(), self.cajaTxt_ap_mat.get(), self.cajaTxt_carrera.get(), self.cajaTxt_area.get(), self.cajaTxt_cel.get(), self.cajaTxt_fechaIni.get(), self.cajaTxt_fechaFin.get(), self.cajaTxt_turno.get(), self.cajaTxt_gestion.get())
    ...
    def fecha_calendario1(self):
        if self.sw_cal == 0:
            ...
            self.btnCal = Button(self, text="Seleccione la Fecha", bg="green", fg="white", command=self.extraccionF1)
    ...
    def fecha_calendario2(self):
        if self.sw_cal2 == 0:
            ...
            self.btnCal2 = Button(self, text="Seleccione la Fecha", bg="green", fg="white", command=self.extraccionF2)
    ...
    def create_widgets(self):
        ...
        self.btnGuardar.place(x=70, y=610)
        self.btnCancelar.place(x=170, y=610)

        frame2 = Frame(self, bg="#d3dde3")  # Frame esta dentro ventana, existe solo en el metodo(No es mienbro de la clase )
        frame2.place(x=305, y=0, width=1030, height=670)
        ...
        self.btnOrdenar1 = Button(frame2, text="Ordenar", command=self.fOrdenar_por_Area, width=10, bg="red", fg="white")
        self.btnOrdenar1.place(x=150, y=45)
        self.var = StringVar(frame2)
        self.var.set('AREA')
        opciones = ['prensa', 'estudio', 'produccion']
        opcion = OptionMenu(frame2, self.var, *opciones)
        opcion.config(width=10)
        opcion.place(x=240, y=45)

        self.btnOrdenar2 = Button(frame2, text="Ordenar", command=self.fOrdenar_por_Carrera, width=10, bg="red", fg="white")
        self.btnOrdenar2.place(x=400, y=45)
        self.var2 = StringVar(frame2)
        self.var2.set('CARRERA')
        opciones2 = ['Informatica', 'Electronica', 'Comunicacion', 'tvu']
        opcion2 = OptionMenu(frame2, self.var2, *opciones2)
        opcion2.config(width=15)
        opcion2.place(x=500, y=45)

        self.btnOrdenar3 = Button(frame2, text="Ordenar", command=self.fOrdenar_por_Gestion, width=10, bg="red", fg="white")
        self.btnOrdenar3.place(x=700, y=45)
        self.var3 = StringVar(frame2)
        self.var3.set('GESTION')
        opciones3 = ['2021', '2022', '2023', '2024', '2025', '2026', '2027', '2028', '2029', '2030'] # agregar mas gestiones
        opcion3 = OptionMenu(frame2, self.var3, *opciones3)
        opcion3.config(width=10)
        opcion3.place(x=800, y=45)
        ...
        frame3.place(x=305, y=100, width=1025, height=210)
        # GRID
        self.grid = ttk.Treeview(frame3, columns=("col1", "col2", "col3", "col4", "col5", "col6", "col7", "col8", "col9", "col10", "col11"))
        ...
        self.grid.place(x=0, y=5, width=1025, height=200)
    ...
    def fOrdenar_por_Gestion(self):
        self.limpiaGrid()
        datosP = self.p1.consulta_por_gestion(self.var3.get())
        for col in datosP:
            # print(col[2]) #pruebita de un solo dato
            self.grid.insert("", END, text=col[0], values=(col[1], col[2], col[3], col[4], col[5], col[6], col[7], col[8], col[9], col[10], col[11]))
